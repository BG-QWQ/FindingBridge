 ...
 *
 * All names use the required `omt_` prefix. Read tools are annotated
 * separately from synchronization tools because sync writes only oh-my-triage's
...
export function registerOMTTools(
  ...
  server.registerTool(
    'omt_summary',
    {
      ...
      description:
        `Return global oh-my-triage database counts with explicit no-data and scope metadata. ${WORKSPACE_CONFIRMATION_DESCRIPTION} For current or latest scanner platform results, call omt_sync_sources before this tool. If no findings are returned, report that fact and do not invent vulnerabilities.`,
      inputSchema: {},
  ...
  server.registerTool(
    'omt_list_findings',
    {
      title: 'List Findings',
      description:
        `List normalized scanner findings with filters, pagination, redacted summaries, and explicit no-data metadata. ${WORKSPACE_CONFIRMATION_DESCRIPTION} rule_id is an exact scanner rule ID match; file_path matches normalized stored finding locations, not scanner project keys, repository names, or current-project selectors. For SonarCloud project keys, use omt_list_source_projects and omt_sync_sources project_keys[source_id]. For current or latest scanner platform results, call omt_sync_sources before this tool. If findings is empty, report that no stored findings matched this request or filters unless a broader unfiltered call or sync proves otherwise; do not invent vulnerabilities.`,

...
**Mission**: FindingBridge is an open-source, self-hosted, privacy-first MCP-based triage layer for existing code/security scanner findings.
...
- All MCP tools must use the `findingbridge_` prefix.
...
- Error messages must be actionable and include next steps (e.g., "Token invalid. Run `findingbridge config set-token github` to update.").
...
- The primary beginner flow is: download release → run executable → **local Web Setup Wizard opens in browser** → guided configuration → generate MCP config.
- CLI terminal wizard (`--cli`) must be available as fallback for headless environments.
...
- Demo mode (`--demo`) is optional and must not replace the real guided setup flow.
